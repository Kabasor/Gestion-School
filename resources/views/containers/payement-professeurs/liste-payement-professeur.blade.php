                    ...
                    <div class="mt-5" >
                        <div class="header">
                            ...
                            <ul class="header-dropdown m-r--5">
                                <li>
                                    <button class="btn-hover color-6"><a href=" {{route('paiementProfesseur.create')}} "> Ajouter</a></button>
                        ...
                        <div class="table-responsive">
                            <table id="tableExport"
                                class="display table table-hover table-checkable order-column m-t-20 width-per-100">
                                    ...
                                    <tbody>
                                        @foreach ($paiementProfesseurs as $paiementProfesseur )
                                            <tr>
                                                {{-- <td> {{$loop->iteration}} </td>
                                                <td> {{$paiementProfesseur->}}</td>
                                                <td> {{$paiementProfesseur->}}</td>
                                                <td> {{$paiementProfesseur->}}</td> --}}
                                                <td >
                                                    <div class="d-flex">
                                                        {{-- <a href="{{route('paiementProfesseur.show',$paiementProfesseur)}}" class="btn btn-tbl-edit">
                                                            <i class="material-icons">remove_red_eye</i>
                                                        </a> --}}
                                                        <a href="{{route('paiementProfesseur.edit',$paiementProfesseur)}}" class="btn btn-tbl-edit">
                                                            <i class="material-icons">create</i>
                                                        </a>
                                                        <form style="display: inline" action="@route('paiementProfesseur.destroy', $paiementProfesseur)" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button class="btn btn-tbl-delete btn-sm delete-confirm" data-name="{{$paiementProfesseur->id}}" type="submit">
